...
export class MatchService {
    ...
    {
        var User1 = await this.userService.getUserQueryOne({ where: { id: Playerid1 }, relations: ["matches"] });
        if (!User1)
            throw "User one not found";
        var User2 = await this.userService.getUserQueryOne({ where: { id: Playerid2 }, relations: ["matches"] });
        if (!User2)
            throw "User two not found";  
        ...
        return ("Match created between" + newMatch.players[0].id + "and" + newMatch.players[1].id);
    }
    ...
    {
		const Match = await this.MatchRepository.findOne({where: {id: id}, relations: ["players"]});
		if (Match === undefined)
